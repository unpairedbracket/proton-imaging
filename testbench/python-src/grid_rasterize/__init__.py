...
def get_intensity(x0, y0, dx, dy, pixel_zoom, xlim, ylim, background_intensity=None, level=0):
    x0 = x0 * pixel_zoom
    y0 = y0 * pixel_zoom
    dx = dx * pixel_zoom
    dy = dy * pixel_zoom
    ...
    edg, ext, secs = rasterize_grid(x0, y0, dx, dy, xlim, ylim, background_intensity, level)
...
def rasterize_grid(x0, y0, dx, dy, xlim=None, ylim=None, background_intensity=None, level=0):
    X = x0 + dx
    Y = y0 + dy
    ...
    seconds = 0
    if level == 0:
        seconds = timeit.timeit("fill_accumulation_py(raster, edges, (X_min, Y_min))", number=1, globals=globals() | locals())
    elif level == 1:
        seconds = timeit.timeit("fill_accumulation_numba(raster, edges, (X_min, Y_min))", number=1, globals=globals() | locals())
    elif level == 2:
        seconds = timeit.timeit("rust_bindings.fill_accumulation_fold(raster, edges, (X_min, Y_min))", number=1, globals=globals() | locals())
    elif level == 3:
        seconds = timeit.timeit("rust_bindings.fill_accumulation_fold2(raster, edges, (X_min, Y_min))", number=1, globals=globals() | locals())
